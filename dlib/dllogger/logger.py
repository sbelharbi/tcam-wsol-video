...
class Logger:
    def __init__(self, backends, master_pid: int, flush_at_log: bool = True):
        self.backends = backends
        self._master_pid = master_pid
        self.starttime = datetime.now()

        self.flush_at_log = flush_at_log
        if not self.flush_at_log:
...
class ArbLogger:
    def __init__(self, backends, is_master: bool, flush_at_log: bool = True):
        self.backends = backends
...
class ArbTextStreamBackend(ArbBackend):
    def __init__(self, verbosity, filename, append_if_exist: bool = False):
        super().__init__(verbosity=verbosity)
        self._filename = filename
        if append_if_exist and os.path.isfile(filename):
